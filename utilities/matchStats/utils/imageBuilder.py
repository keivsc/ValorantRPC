# Code from https://github.com/colinhartigan/ggsheet
...
class Builder:
    ...
    def __init__(self, game_data, appdata):
        self.appdata = appdata
        self.cur_path = self.appdata.get_path(os.path.join(os.getenv('APPDATA'), 'ValorantRPC/matchStats'))
        self.base_image = os.path.join(self.cur_path,"data/ggsheetreal.png")
        self.output_folder = os.path.join(self.cur_path,"matches")

        self.game_data = game_data
        
        self.img = Image.open(self.base_image)
        ...
        self.fonts = {
            "ddin": {
                "mvp_agent": ImageFont.truetype(os.path.join(self.cur_path,"data/fonts/DINNextLTPro-Medium.ttf"), 28),
                "player_agent": ImageFont.truetype(os.path.join(self.cur_path,"data/fonts/DINNextLTPro-Medium.ttf"), 14),
                "map_label": ImageFont.truetype(os.path.join(self.cur_path,"data/fonts/DINNextLTPro-Regular.ttf"), 20),
                "map_text": ImageFont.truetype(os.path.join(self.cur_path,"data/fonts/DINNextLTPro-Medium.ttf"), 30),
                "win_loss_label": ImageFont.truetype(os.path.join(self.cur_path,"data/fonts/DINNextLTPro-Medium.ttf"), 25),
                "player_stat_label": ImageFont.truetype(os.path.join(self.cur_path,"data/fonts/DINNextLTPro-Regular.ttf"), 12),
                "timestamp": ImageFont.truetype(os.path.join(self.cur_path,"data/fonts/DINNextLTPro-Regular.ttf"), 22),
            },
            "tungsten": {
                "mvp_player": ImageFont.truetype(os.path.join(self.cur_path,"data/fonts/Tungsten-Bold.ttf"), 90),
                "mvp_stats": ImageFont.truetype(os.path.join(self.cur_path,"data/fonts/Tungsten-Bold.ttf"), 65),
                "mvp_label": ImageFont.truetype(os.path.join(self.cur_path,"data/fonts/Tungsten-Bold.ttf"), 35),
                "header_scores": ImageFont.truetype(os.path.join(self.cur_path,"data/fonts/Tungsten-Bold.ttf"), 165),
                "header_team_name": ImageFont.truetype(os.path.join(self.cur_path,"data/fonts/Tungsten-Bold.ttf"), 120),
                "player_name": ImageFont.truetype(os.path.join(self.cur_path,"data/fonts/Tungsten-Bold.ttf"), 40),
                "player_stats": ImageFont.truetype(os.path.join(self.cur_path,"data/fonts/Tungsten-Bold.ttf"), 42),
            }
        }

        self.other_side_offsets = {
            "mvps": {
                "text": 1116,
                "images": 368,
                "overrides": {
            ...
            "header_footer": {
                "text": {
                    "map_label": {
                        ...
                        "dimensions": (64,14), 
                        "color": (255,255,255),
                        "font": self.fonts["ddin"]["map_label"],
                        "text": "MAP",
                        "upper": True,
                    },
                    "map_name": {
                    ...
                    "timestamp": {
                        ...
                        "dimensions": (575, 35), 
                        "color": (138,148,156),
                        "font": self.fonts["ddin"]["timestamp"],
            ...
            "mvps": {
                "text": {
                    ...
                    "mvp_label": {
                        ...
                        "dimensions": (72,41),
                        "color": (255,255,255),
                        "font": self.fonts["tungsten"]["mvp_label"],
                        "text": "MVP",
                        "justify": "c"
                    }
                },
            ...
            "players": {
                "text": {
                    ...
                    "combatscore_label": {
                        ...
                        "dimensions": (174, 26), 
                        "color": (138,148,156),
                        "font": self.fonts["ddin"]["player_stat_label"],
                        "text": "combat score",
                        "upper": True,
                        "justify": "l"
                    },
    ...
    def draw_header_footer(self):
        ...
        for img_type, image in refs["images"].items():
            new_img = Image.open(os.path.join(self.cur_path,*image["file_path"].format(map=self.game_data['match_map_display_name'].lower(),mode=self.game_data['match_mode'].lower()).split("/"))).convert("RGBA")
    ...
    def draw_players(self):
        ...
        for team_id,team in enumerate(self.game_data["players"]):
            ...
            if team_id != 0:
                # team offset (players)
                offset = self.other_side_offsets["players"]["team"]
                ...
                # team offset (mvps)
                text_offset = self.other_side_offsets["mvps"]["text"]
                image_offset = self.other_side_offsets["mvps"]["images"]
                for ref,data in mvp_refs["text"].items():
                    if self.other_side_offsets["mvps"]["overrides"].get(ref):
                        data["anchor"] = (data["anchor"][0]+self.other_side_offsets["mvps"]["overrides"][ref],data["anchor"][1])
                    else:
            ...
            for position,player in enumerate(team):
                ...
                if position == 0:
                    ...
                    for img_type,image in mvp_refs["images"].items():
                        new_img = None
                        if img_type == "agent": 
                            new_img = Image.open(os.path.join(self.cur_path,*image["file_path"].format(agent=player['agent_display_name']).split("/"))).convert("RGBA")
                            ...
                            new_img = Image.open(os.path.join(self.cur_path,*image['file_path'].format(side=team_id).split("/"))).convert("RGBA")
                            self.__draw_image(image,new_img)
                    ...
                    for img_type,image in player_refs["images"].items():
                        ...
                        if img_type == "agent": 
                            ...
                            new_img = Image.open(os.path.join(self.cur_path,*image["file_path"].split("/"))).convert("RGBA")
    ...
    def build_image(self):
        ...
        self.img.save(os.path.join(self.cur_path,f"output/{self.game_data['match_id']}.png"))
        return os.path.join(self.cur_path,f"output/{self.game_data['match_id']}.png")
